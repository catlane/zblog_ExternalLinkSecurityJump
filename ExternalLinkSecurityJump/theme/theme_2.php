<?php
$jump_url = rtrim ($_GET[ 'jump_url' ],'/');
$auto_jump_time = $zbp->Config ( 'ExternalLinkSecurityJump' )->auto_jump_time;

$urlData = $zbp->Config ( 'ExternalLinkSecurityJump' )->data;
$urlData = is_array ($urlData) ? $urlData : [];
$isHave = false;
foreach ( $urlData as $v ) {
    if ( strpos ( $jump_url , $v[ 'url' ] ) !== false ) {//存在
	   $isHave = true;
        continue;
    }
}
...
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
	...
	<title><?php echo $isHave ? '安全链接' : '未验证的链接';?> - <?php echo $GLOBALS['blogname'];?></title>
<style>
body {
	margin: 0;
	padding: 0;
...
<div id="js-alert-box" class="alert-box">
  ...
  <div class="alert-body">
	  <?php if ( $isHave === true ) {//自动跳转 ?>
		  <div id="js-alert-head" class="alert-head">正在为您跳转...</div>
		  <div class="alert-concent">
		      <p>若您的浏览器未能在<?php echo $auto_jump_time;?>秒内自动跳转</p>
		      <p>点击按钮手动跳转</p>
		    </div>
      <?php }else{//没有 ?>
		  <div id="js-alert-head" class="alert-head">未验证链接</div>
		  <div class="alert-concent">
		      <p>该跳转链接不属于本站验证链接</p>
		      <p>可能包含未知的安全隐患</p>
		    </div>
    ...
    <a id="js-alert-btn" class="alert-btn" href="<?php echo $jump_url;?>">立即前往</a>
...
function alertSet ( e ) {
    ...
    var t = <?php echo $auto_jump_time;?>,
        n = document.getElementById ( "js-sec-circle" );
    document.getElementById ( "js-sec-text" ).innerHTML = t,
        setInterval ( function () {
                if ( 0 == t ) {
                    location.href = "<?php echo $jump_url;?>";//#时间到后跳转地址
                } else {
                    t -= 1,
                        document.getElementById ( "js-sec-text" ).innerHTML = t;
                    var e = Math.round ( t / <?php echo $auto_jump_time;?> * 735 );
